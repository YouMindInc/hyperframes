...
- `/product-launch-video` — End-to-end orchestrator that turns a URL into a 60-90s launch / SaaS explainer / promo video. Internally runs `(web-research ‖ design-system) → story-design → (audio ‖ visual-design) → hyperframes build`, where the LLM subagents are web-research, design-system, story-design, visual-design, plus the Phase 4 scene workers and finalize. **`design-system` (Phase 1b) extracts the target site's brand tokens into `design.html`** — the single source of truth for palette, typography, and motion eases consumed by Phase 3 and Phase 4b. **Phase 2.5 (audio) and Phase 4a (prep) are deterministic Node scripts, not subagents** — `scripts/audio.mjs` and `scripts/prep.mjs`. The build is split into `4a prep → 4b scene workers (parallel fan-out, ≤2 scenes/group) → 4c finalize`, and **finalize itself owns the render** (lint → validate → inspect → snapshot → render + ffprobe verify) — there is no separate Phase 5. **Phase 1 ‖ Phase 1b run in parallel** (two Agent blocks in one orchestrator message, both `run_in_background: true`); **Phase 2.5 runs in parallel with Phase 3** (one Bash + one Agent in the same orchestrator message, both `run_in_background: true`); `audio.mjs` spawns Lyria BGM **detached** and returns the moment voice + transcribe are done, so BGM never blocks the build (Phase 4c re-checks `bgm.wav` on disk before emitting `<audio>`). Phase 4a's `prep.mjs` packs scenes into worker groups using the `**Continuity:**` / `**PrimaryAsset:**` anchors that Phase 3 writes into `section_plan.md`. Phase guides / scripts / archetypes live at `skills/product-launch-video/phases/<phase>/`; subagent wrappers at `skills/product-launch-video/agents/<phase>.md`. Audio is optional and gracefully degrades: TTS prefers `$ELEVENLABS_API_KEY` and falls back to local Kokoro (`npx hyperframes tts`); BGM via Google Lyria requires `$GOOGLE_API_KEY` (skipped if absent).
