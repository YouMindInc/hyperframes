...
| `design-system/chunks/tokens.css`                   | Brand DNA (`--font-display`/`--font-body`/`--brand-primary`/`--canvas`/`--ink` + surface aliases). Inlined wholesale into `captions.html` as `<style data-brand-tokens>`.                                                                                                     |
