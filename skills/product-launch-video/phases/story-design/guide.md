...
#### `continuity` — `"break"` | `"continue"`（叙事连续性，**驱动下游分组**）

这一缝隙是不是"同一镜头/同一视觉世界的延续"。**纯叙事判断**，和用什么 ease 无关：

- `continue` —— 这一场承接上一场（同素材延续、共享元素跨场、镜头连续推进）。下游 `prep.mjs` 会把 `continue` 的相邻场景**分到同一个 scene worker**（这样一个 worker 能同时写两场、做真正的共享元素衔接）。
- `break` —— 全新起手，不承接前一场（类型/调性切换、硬切）。下游开一个新 worker。
- **Scene 1 永远是 `break`**（无前序场景可承接）。

> 这是 story-design 的判断，不再由下游 visual-design 从散文反推 —— 你在写这一场时就知道它连不连续，直接写出来。

#### `intent` — 5 种叙事衔接意图（**不是**视觉实现）

从这 5 个里选一个。这是"叙事级"词汇 —— 表达"这缝隙是哪种衔接"，**不碰** blur 量 / 方向 / 时长（那些是 visual-design 按 preset/palette 翻译的事）：

| Intent     | 叙事含义                                                        | 典型搭配 continuity | 下游翻译方向（visual-design 决定具体值）     |
| ---------- | --------------------------------------------------------------- | ------------------- | -------------------------------------------- |
| `morph`    | **一个共享元素跨场变形**（卡片→头像、波形→搜索框、手机簇→圆环） | **必须 `continue`** | Tier-A 共享元素桥接（worker 在两场写 morph） |
| `cut`      | 利落切换，两场不连续（类型/调性切换、高能瞬间）                 | 通常 `break`        | hard cut / crossfade                         |
| `slide`    | 方向性滑动 / 推移（匹配叙事流向：前进、下一点）                 | 任意                | push-slide（方向由 visual-design 定）        |
| `dissolve` | 柔性溶解 / 焦点拉移（进入氛围场、情绪过渡）                     | 任意                | crossfade / blur-crossfade（撞色时）         |
| `zoom`     | 镜头推进 / 缩放穿越到下一焦点                                   | 任意                | zoom-through                                 |
...
- 每个 scene 是否都有 `transition`，并包含 `continuity`（break/continue）、`intent`（5 个意图之一）、`description`（10-30 words）？`intent: morph` 的是否带了 `sharedMotif`，且 `continuity: continue`？Scene 1 是否 `continuity: break`？
...
- 每个 scene 都必须有 `transition` 字段（`continuity` + `intent` + `description`，morph 时加 `sharedMotif`），包括 scene 1（`continuity: "break"` + `intent: "cut"`）。
