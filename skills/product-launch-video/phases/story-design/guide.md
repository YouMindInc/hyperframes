...
> 因此 `continuity` 不是自由叙事字段，而是**跟着 `intent` 走**：`morph` ⇒ `continue`，其余一律 `break`。`validate-narrator-scripts.mjs` 强制这条（违反 → fatal）。这么收窄是为了不让一个"叙事上感觉连续"的 slide 白占 cap=2 的 worker 名额、把真正需要 `continue` 的 morph 对挤出分组窗口（那是下游头号 fatal）。
...
- **`intent: "morph"` ⟺ `continuity: "continue"`**（且 `sharedMotif` 非空）；`cut` / `slide` / `dissolve` / `zoom` 一律 `continuity: "break"`。`validate-narrator-scripts.mjs` 强制（违反 → fatal）。
