...
- `pip install transformers torch soundfile numpy` 已装（本地，免费）→ MusicGen via HuggingFace transformers 本地生成（首次运行下载 ~300MB 模型）。脚本会在 TTS 跑的同时后台 pip-install 缺失的包；**本地 MusicGen 会按 TTS 实测总时长分段生成（默认 ≤8s/段）再合成一个 `assets/bgm.wav`**，避免 30-60s 一次性生成触发模型 token/position 上限。
...
- `--bgm-segment-seconds <n>` — 本地 MusicGen 分段长度（默认 8，夹在 2-12；一般不用改）

exit 0 → voice + transcribe 完成（BGM 可能仍在后台渲染，`audio_meta.json` 会记录 `bgm_log` / `bgm_pid` / 分段信息），继续。
...
4. 解析 `section_plan.md` 的 anchors：必选 Effects / Duration / Continuity，可选 Blueprint / Components / Surface / **SFX**
...
Step 6 预飞（check-compositions.mjs）退 0 后，编排器先做**确定性 Bash 前置**（wait-bgm + assemble/sfx-verify + preflight），再派一个**修复型 finalize subagent** 包圆视觉 QA → 就地修 → render。设计取舍：能确定性做的都不要让 agent 做 —— BGM 落盘判断、拼装、SFX 复核、lint/validate/inspect、caption keep-out 扫描都搬到 Bash；agent 只负责 snapshot 眼检 + 就地一次改对 + render。`compositions/scene_N.html` 是 worker 的源文件，改它 = 改源；**不**回退重派 worker（重派 = 冷启 + 重读 + 重写 200 行 = 最慢路径，只留给"需重新构图"的例外）。
