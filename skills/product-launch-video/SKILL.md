...
4. **Bash 直跑**。Step 1 按输入类型二选一,**`derive-context-pack` + `build-design --no-emit` 两步两条路共用**（design-system 直接吃 capture 产物）：

**(A) URL 输入** —— hyperframes capture：

```bash
(cd "$PROJECT_DIR" && npx hyperframes capture "<TARGET_URL>" -o ./capture)
```

**(B) script / brief 输入（无 URL，纯文字片 / 自带脚本）** —— 不抓取，合成一份最小 capture 喂给同一条下游。**preset 由你（master）选定**（无站点可推断；按用户意图从 19 个里挑，或问一句）；用户的脚本/brief 全文落进 `visible-text.txt`；`colors:[]` 让 build-design 走 **R2 preset-palette 回退**（无需品牌色也有完整可读调色板；用户若指定品牌色就填进 `colors`，会覆盖 preset 默认）：
...
(cd "$PROJECT_DIR" && printf '%s\n' "<用户脚本 / brief 全文>" > capture/extracted/visible-text.txt)
```

> B 路把"自带脚本"和"无素材抓取"一次满足：narrator_scripts 仍由 Step 2 story-design 从 brief 生成（场景一律 `assetCandidates: []`，纯文字/排版片）；若用户**已有成稿 narrator_scripts.json**，放进 `$PROJECT_DIR/` 即可，Resume 表会跳过 story-design。

**两条路共用下游**（B 路 build-design **必须带 `--style <chosen-preset>`** 强制 preset；A 路省略走自动推断）：

```bash
(cd "$PROJECT_DIR" && node <SKILL_DIR>/scripts/derive-context-pack.mjs --capture ./capture)
# 确定性产出 design-system/inference.json（site_dna + preset 评分 / forced）。Step 1b 与 Step 2 都读它且并行 fork。
(cd "$PROJECT_DIR" && node <SKILL_DIR>/phases/design-system/scripts/build-design.mjs ./design-system --no-emit)   # B 路：追加 --style <chosen-preset>
```
...
- N 个 `Agent`（`subagent_type: "general-purpose"`，每个 `run_in_background: true`）。prompt = `cat agents/hyperframes-scene.md` 全文 + `## Dispatch context`，原样透传。dispatch context 的 top-level：`SKILL_DIR` / `PROJECT_DIR` / `Worker ID` / `Captions: <enabled|disabled>`（= group_spec.captions_enabled）/ `Dispatch packet: /tmp/scene-dispatch/w<N>.txt`，外加 `## Tokens/easings/voice`（共享头三份正文）+ `Scenes:` 列表两段（即 packet 内容）。
  ...
  **`Scenes:` 列表逐字段从 `group_spec.json.groups[i].scenes[<sid>]` verbatim 抄**（只放该 worker 的 1-2 个 scene）：`scene_id` / `effects` / `rule_paths` / `assetCandidates` / `estimatedDuration_s` / `voicePath` / `blueprint` / `shared_element_bridge`（Tier-A，多数 null）/ `design_chunks`（含整份组件库的绝对路径——worker 按视觉判断自选）/ `creative_brief`（Phase 3 该 scene 散文）。字段语义见 `agents/hyperframes-scene.md`。
...
所有 subagent（scene workers + captions）都返回后，跑预飞 harness。注意 `check-compositions.mjs` **只按 `group_spec.json` 的 `scene_ids` 扫 `compositions/scene_*.html`**（跑 scene 专属规则：root div 契约 / selector 作用域等）——它 **不检 `captions.html`**（captions.html 走 caption agent 自己的 self-lint + 结构 grep + Step 7 finalize 的整项目 lint）：
...
preflight 把 agent 不需判断的事一次做完，全写进 `finalize_brief.json`：warm 一个 pinned `npx hyperframes@<version>` cache、用它跑 lint/validate/inspect 抓 tail + summary counts、算 snapshot 时刻表、（captions_enabled 时）跑 `captions.mjs keepout` 静态查"foreground 下沿 y ≤ 900"、跑 `check-rendered-perception.mjs` 做 puppeteer 真渲几何检查（含 `cross-text-collision` —— 抓多文字 bbox 互撞）。**keep-out 违规会带准 Edit 字符串**（`edit_old`/`edit_new`），finalize 直接 `Edit` 改对、不读源不算几何。brief 字段（`preflight_clean` / `gates_clean` / `gates.*` / `bgm.*` / `caption_keepout.*` / `perception.*` / `anomalies[]` / `snapshot_times_s[]` / `npx_prefix` / `scenes[]`）+ 算法细节见 `preflight-finalize.mjs` 头。

**退出码（新行为，编排器必读）**：

- **exit 0** → gates 全过 → 进 (3) 派 finalize。
- **exit 2** → **BLOCKING** —— lint / validate / inspect 至少一道有真 ERROR（exit_code ≠ 0）。**不要**派 finalize，**不要** `--allow-gate-failure` 绕开。Read `finalize_brief.json.gates.<gate>.output_tail` 定位是哪个 scene 哪类违规：
  - `text_box_overflow` / `canvas_overflow` / `container_overflow` → worker 凭直觉算 layout、实际包行 / 越框 → 多数情况**回 Step 6 重派该 worker**（附上 inspect 报的 selector + scene 让 worker 知道哪儿撞了 / 用 relative flow 而非 absolute 钉死）；少数 by-design 越框 → 给元素加 `data-layout-allow-overflow="true"` 然后重跑 preflight
  - `lint` / `validate` 失败 → 多数是 scene 文件 schema / selector / 资产路径错 → 看 tail 定位文件 + `Edit` 改对 + 重跑 preflight
  - 想看 finalize 怎么自己处理这些（用于 agent-flow debug）→ 再跑 `preflight-finalize.mjs --allow-gate-failure`
- **exit 1** → preflight 自己崩了（参数错 / group_spec 缺）—— 看 stderr 修 invocation。

**`anomalies[]` 数组**（即使 preflight 退 0 也要扫一眼）：响亮但不阻塞的预警。当前最常见的一条是 `perception_check_skipped`（本地没装 puppeteer → cross-text-collision / depth-layer-ghost 等真渲几何检查整条没跑 → finalize 的 snapshot 眼检成了唯一兜底，遇 rate-limit 即一票否决）。每条 anomaly 带 `actionable_install_command`，照抄一次性装好下次就有覆盖。
