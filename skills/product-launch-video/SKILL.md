...
- **design-system**（Phase 1b）：`Agent`（`subagent_type: "general-purpose"`），prompt = `agents/design-system.md` 内容 + `## Dispatch context`（含 `SKILL_DIR`、`PROJECT_DIR`、`Target URL`）。**可选省一次 Read**：把 Step 1 已产的 `inference.json` 整文 `cat` 进 dispatch（`(cd "$PROJECT_DIR" && cat design-system/inference.json)`，~2-4 KB），agent 即可直接用 `confidence` / 候选评分 / `brand.needs_review` 选 preset，无需开工先 Read 它（但 agent 仍需 Read `guide.md` 拿 §1 命令模板 / §3b 截图流程 / §4 报告模板；`brand.needs_review=true` 时仍 Read 截图做视觉判断）。流程分四步：
  1. 选 preset。
  2. `brand.needs_review=true` 时看截图裁品牌色。
  3. `build-design.mjs --style <chosen> [--brand-primary <hex>]`。**不再自己跑 `--no-emit`**（已在 Step 1 跑过；仅 capability auto-install 后需重验时才重跑）。
  4. `emit-chunks.mjs`。产 `design.html` + `chunks/`。

  选中的 preset 若在 `style-presets/<preset>/` 放了 `caption-skin.html`，`emit-chunks` 会把它拷进 `chunks/caption-skin.html`，且 design.html 自动多一段 **§C 实时预览**（内嵌该皮肤循环播放）。Captions 皮肤来源：preset 自带的 `caption-skin.html` 优先，否则按 registry 评分选，详见 Step 5.5。
...
# 这只是乐观估计（偏 enabled ⇒ 给 plan 留底部字幕带）：即便它与 Step 5 prep 的权威闸不一致也安全，
# 因为 Step 6/Step 7 的 keep-out 一律以 group_spec.captions_enabled 为准。
...
- N 个 `Agent`（`subagent_type: "general-purpose"`，每个 `run_in_background: true`），prompt = `agents/hyperframes-scene.md` 全文 + `## Dispatch context`：
  ...
  Scenes:
    - scene_id: scene_<N>
      ...
      shared_element_bridge: null | { bridge_id, role: from|to, partner: <scene_id>, seam_duration_s }   # Tier-A 桥接（见 hyperframes-scene 约束 #14）；多数 scene 是 null
      design_chunks:
...
(cd "$PROJECT_DIR" && node <SKILL_DIR>/scripts/check-bridge-continuity.mjs \
  --hyperframes . \
  --group-spec ./group_spec.json)
```

`check-bridge-continuity.mjs` 对每条 Tier-A 桥接，确定性校验出场+进场两场都有同 `data-bridge-id` 元素且没被静态藏掉（写 `bridge_check.json`）。交接姿态的**视觉对齐**静态查不到（在 GSAP 末态几何里），由 Step 7 finalize 在接缝 snapshot 眼检。
...
- captions（Phase 4a.5）：build-captions.mjs 的 groups/words/split 统计（`caption_groups.json.stats`）；build-captions-html.mjs 选的皮肤 `caption-<x>`、是否生成 captions.html、node 结构自检结果（OK/失败）；或 skipped 原因
...
| `group_spec.json` 有，`compositions/scene_*.html` 缺 **或** captions 链未跑过（`caption_groups.json` 缺）   | Step 5.5+6（先 Bash 跑 `build-captions.mjs` → `build-captions-html.mjs` 产 `caption_groups.json` + `captions.html`；再同条 message 并行：缺哪个 scene 派 worker）。**captions 已跑过的判据 = `caption_groups.json` 存在**（无论它产出 `captions.html` 还是合法 skip）—— 不要用 `captions.html` 缺当判据：captions 合法 skip（词被清洗光 / `tokens.css` 缺 / 无 words）时本就不产 `captions.html`，用它当判据会每次 resume 重跑又重 skip。 |
...
> ❌ Step 6 的 N 个 scene worker subagent 必须同一条 message 里 fan-out，每个 `run_in_background: true`。先起一个等完成再起另一个 = 串行化反模式（GitHub issue #29181 的默认行为，请刻意对抗）。Captions 在 fan-out 之前由 Bash 脚本（build-captions.mjs → build-captions-html.mjs）确定性产出，**不占 subagent**。下游 gate 报错回上游修，不在 finalize 里 patch；具体 invariant 见各 agent prompt。
