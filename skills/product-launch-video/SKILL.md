...
agent 跑 `build-design.mjs --no-emit` → review `inference.json` → 选 preset → `build-design.mjs --style <chosen>` → `emit-chunks.mjs`。Captions 的样式不在这里决定 —— 改由 Step 5.5 的 captions agent 从 6 个逐词 karaoke `caption-*` registry 皮肤里按 `inference.json` vibe 选 1 个，tokenize 成 brand-strict + 逐 scene 背景适配后写 `compositions/captions.html`（分组/计时由确定性引擎 `build-captions.mjs` 先算好 `caption_groups.json`，agent 只做选皮肤 + 组装）。
...
audio 完成且 `audio_meta.json` 存在后，读取 effects-catalog、blueprints-index、SFX 库（三者全部内联进 dispatch，subagent 不再各自 Read）：
...
- visual-design：`Agent`（`subagent_type: "general-purpose"`），prompt = `agents/visual-design.md` 内容 + `## Dispatch context`：
  ...
  ## SFX library
  <manifest.json 全文 —— SFX 可选；列出每条 file / duration / 用途，agent 不必自己 Read>
  ```
...
**先跑确定性引擎**（prep 退 0 后、fan-out 前，Bash 直跑，几十毫秒）：
...
它把 whisper 词流做完 **清洗 / 分组 / 打 class / 全局计时 / scene+surface 归属** → `caption_groups.json`。退 0 即可（无 group_spec / 无 whisper words / 无 tokens.css 时它打印 `captions: skipped (<reason>)` 也退 0 —— 那种情况下不派 captions agent，finalize 不挂 track-12）。

然后读 `group_spec.json.groups[]` 得 worker 数 N。**同一条 message** 里并行启动 **subagent**：N 个 scene worker +（`caption_groups.json` 存在且非空时）1 个 captions agent。captions agent 跟 scene worker 是不同物种的 sub-comp（一个全片字幕、一个单 scene 画面）：scene worker 依赖 `group_spec.json` + `chunks/`，captions agent 依赖 `caption_groups.json` + `chunks/tokens.css` + 一个 registry caption 皮肤，输出不同文件，可以无锁并行。
...
退出条件：写出 `compositions/captions.html` 或汇报 `skipped` —— finalize 看文件存在性决定是否挂 track-12 clip。captions agent 的 DONE gate 是 **浏览器 self-lint（可见性回归零 warning）+ 结构 grep**（见 `phases/captions/guide.md` §7）；**不跑** `npx hyperframes lint compositions/captions.html`（lint 参数是项目目录，传文件必退 1，会被误判成失败导致 captions 静默不出）。captions.html 的整项目 lint 由 Step 7 finalize 的 `npx hyperframes lint`（无文件参数）覆盖。self-lint / 结构 grep 真失败 → captions agent STOP 且不写出 captions.html，finalize 自动跳过。
...
- captions（Phase 4a.5）：build-captions.mjs 的 groups/words/split 统计（`caption_groups.json.stats`）；captions agent 选的皮肤 `caption-<x>`、是否生成 captions.html、surfaces 适配数；或 skipped 原因
...
| `group_spec.json` 有，`compositions/scene_*.html` 缺 / `captions.html` 缺（且 ≥1 scene 有 `wordsPath`）     | Step 5.5+6（先跑 `build-captions.mjs` 产 `caption_groups.json`；同条 message 并行：缺哪个 scene 派 worker，captions.html 缺且 `caption_groups.json` 非空就同时派 captions agent） |
