...
Step 6 预飞（check-compositions.mjs）退 0 后，编排器先做**两步确定性 Bash 前置**（assemble + preflight），再派一个**修复型 finalize subagent** 包圆视觉 QA → 就地修 → render。设计取舍：能确定性做的都不要让 agent 做 —— 拼装是纯函数（assemble-index），lint/validate/inspect 是无判断的复核（preflight），都搬到 Bash；agent 只负责 snapshot 眼检 + 就地一次改对 + render。`compositions/scene_N.html` 是 worker 的源文件，改它 = 改源；**不**回退重派 worker（重派 = 冷启 + 重读 + 重写 200 行 = 最慢路径，只留给"需重新构图"的例外）。
...
- finalize（Step 7，Phase 4c）：assemble-index 摘要（clips/voice/bgm/captions/sfx 计数）、preflight 摘要（pinned hyperframes 版本 / gates_clean / 任何 deterministic fix）、lint/validate/inspect 状态、snapshot 眼检每 scene 一行、**就地修过的 scene 文件（file + 改了什么）**、verify-render 的 mp4 路径 + 字节数 + ffprobe duration、quality、任何（例外）重派的 worker
