...
**输入（INPUT）：** `<PROJECT_DIR>/research/context_pack.md`（主输入）· `<PROJECT_DIR>/research/assets/`（asset pool）
...
**Path contract**：Dispatch 给 `PROJECT_DIR`（视频项目根）。读 `<PROJECT_DIR>/research/context_pack.md`，写 `<PROJECT_DIR>/narrator_scripts.json`，Bash 用 `(cd "$PROJECT_DIR" && ...)` subshell。
