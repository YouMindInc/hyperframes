...
3. **Token / 组件来自 `design_chunks`，逐字粘贴**
   ...
   - **字体引用必须走 token，不要硬编码字体名**：
     ...
     - **永不**写 `font-family: 'Inter', ...` / `font-family: 'Playfair Display', ...` —— 这会绕过 `index.html` 已注入的 `@font-face`，让站点真实字体无效。如果 `chunks/tokens.css` 缺 `--font-*` 某个角色 token，照样用 `var(--font-body)` 兜底（CSS 解析失败时会用 root 默认），不要降级到字面字体名
...
# 4) 字体名必须走 var(--font-*) token —— 硬编码字面字体名会绕过 index.html <head> 的 @font-face
