...
| hyperframes-scene    | N×subagent（general-purpose，并行） | `compositions/scene_*.html`             | `agents/hyperframes-scene.md`         |
...
agent 跑 `build-design.mjs --no-emit` → review `inference.json` → 选 preset → `build-design.mjs --style <chosen>` → `emit-chunks.mjs` → 再选 caption-\* registry component 写 `caption-style.json`（详见 `phases/design-system/guide.md` §5；prep 阶段读这个文件并 `npx hyperframes add` 装到 `compositions/components/`，captions.mjs 用它合成 captions.html）。
...
10. **Captions**：当 `audio_meta.json` 存在且 `design-system/caption-style.json` 存在（design-system agent 在 Step 1b 选好的 registry caption-\* component 名），先跑 `npx hyperframes add <caption-style>` 装 component 到 `compositions/components/`，再调用 `<captionsBuilder>` patch 它的 WORDS / RAW_GROUPS + 注入 `chunks/tokens.css` brand DNA + 替换 composition id，输出 `compositions/captions.html`（单条 full-bleed sub-comp，覆盖整个时长）
...
| `group_spec.json` 有，`compositions/` 空或缺 scene            | Step 6（scene workers，缺哪个 scene 派哪个） |
| 所有 `compositions/scene_*.html` 齐，`renders/video.mp4` 缺失 | Step 7（finalize）                           |
| `renders/video.mp4` 有                                        | 报告已完成，停止                             |

---

> ❌ Step 6 的 N 个 worker 必须同一条 message 里 fan-out，每个 `run_in_background: true`。先起一个等完成再起另一个 = 串行化反模式（GitHub issue #29181 的默认行为，请刻意对抗）。Step 1 现在是单条 capture（design-system 直接吃 capture 产物，不再并行抓两次）。
