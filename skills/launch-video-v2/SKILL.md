...
- 仅有 URL 时可先用域名/页面标题；`research/` 写入前可以改名，写入后定型
...
4. **同一条 message 里并行启动**：
   - web-research：`Bash` + `run_in_background: true`，在 `PROJECT_DIR` 内按 `phases/web-research/guide.md` 执行。
   - design-system：`Agent` + `run_in_background: true`，prompt = `agents/design-system.md` 内容 + `## Dispatch context`（含 `SKILL_DIR`、`PROJECT_DIR`、`Target URL`）。
