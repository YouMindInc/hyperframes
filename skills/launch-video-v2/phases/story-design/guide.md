...
- **`path`** — 必须精确为 `public/<basename>`。`<basename>` 直接从 Asset Inventory 条目中读取（格式为 `assets/001-xxx.png`），去掉 `assets/` 前缀，加 `public/` 前缀。例如 `assets/022-2f1c0ba7-1260x944.png` → `"public/022-2f1c0ba7-1260x944.png"`。Phase 4a 会把 `research/assets/` 复制到视频项目根的 `public/`。
  ...
  "scenes": [
    {
      ...
      "script": "纯文本旁白，可以嵌入 <em>/<brand>/<emph>/<cta> 标签为 caption 渲染加 hook。当视觉承载信息时可以是空字符串。",
      "captions": [
        "Cards here,",
        "expenses there.",
        "<brand>Brex</brand> built for",
        "<em>speed</em> and control."
      ],
...
### Caption 切组（可选，但有 narration 的 scene 强烈建议写）

为每个 scene 写 `captions: string[]`。**每个字符串就是屏幕上一行 caption**（一组、2-3 个词）。captions.mjs 会按你的字面分组对齐到 audio 出来的 word timestamps，做 karaoke 高亮 + 单行硬切。

切组时遵守 4 条**硬规则**：

1. **每组 2-3 个词** —— 超过 3 个词 stamp 落不下来。短组 = 高能；长组 = 抒情
2. **组的总和必须等于 script 去标签后的所有词** —— 不能跳词、不能合词、不能改大小写以外的拼写
3. **句末标点切组** —— `.` `?` `!` `,` `—` 之后必须断
4. **每组顺序与 script 顺序一致** —— 不能重排

切组**软指南**（影响节奏感）：

- 高能 / hook scene → 短组（2 词/组、快翻）
- 抒情 / proof scene → 长组（3 词/组、慢翻）
- 标点已天然切了，但**意义停顿**也可以切（"AI moves" / "fast." 比 "AI moves fast." 更有节奏）

### Inline tag —— 给单个 word 加 caption 视觉 hook

在 caption 字符串里给 word **包标签**就能加视觉效果。tag 包裹的 inner text 仍然是 caption word 的字面文本。可用 4 类标签：

| 标签      | 视觉效果                                                                                                      | 何时用                                                                             |
| --------- | ------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `<em>`    | **Script-flick**（peoples 招牌）：字体换成 `var(--font-script)`（Caveat Brush），红色，斜 −3°，无 drop shadow | "human aside" 词——和大写 slab 形成"两个声音"对比。**一组 ≤1 次**，全片 ≤30% 的组有 |
| `<brand>` | 橙色、字号 +8%                                                                                                | 品牌 / 产品名（"Brex"、"HeyGen"）。**每次出现都包**，全片每个 brand name 至少一处  |
| `<emph>`  | 红色 ink（不换字体不旋转，比 `<em>` 克制）                                                                    | 情绪关键词（"finally"、"actually"、"matters"）。**全片 ≤ 5 次**，否则失去强调感    |
| `<cta>`   | 红色 4px 下划线                                                                                               | CTA 词（"Try it"、"Start now"、"Talk to sales"）。**只在 CTA scene 出现**          |

ALL CAPS 词（≥2 字母全大写）和 numeric 词（开头是数字）由 captions.mjs **自动检测**加 class——不要手动包标签。

**注意**：

- TTS 自动 strip 这 4 类标签后再读（不会把 "em" / "brand" 等读出来）
- 一个词只能包一个标签
- 标签内不能套标签（`<em><brand>X</brand></em>` 非法）
- script 字段也可以嵌入相同标签——captions.mjs 优先读 captions 字段；缺失 / 对不齐 whisper word grid → 从 whisper 自动切组兜底（无 tag）

### Caption 切组样例（with tags）

```json
"sceneNumber": 2,
"script": "Meet <brand>Brex</brand>. Finance built for speed and <em>control</em> — across 120+ countries.",
"captions": [
  "Meet <brand>Brex</brand>.",
  "Finance built",
  "for speed",
  "and <em>control</em> —",
  "across 120+",
...
—— 高能开局短组（2 词），中段抒情（3 词），CTA scene 收尾长组。`120+` 自动检 cap-num（橙色 bold）。`Brex` 用 `<brand>`（橙、稍大）。`control` 用 `<em>`（红色手写斜体——这一拍是整 scene 的灵魂）。
