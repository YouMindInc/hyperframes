...
<anchor-at-target path="rules/anchor-at-target.md">Land element A on element B's anchor without manual delta math. Static CSS aligns A's `top`/`left` to B's anchor, then tween only `x`/`y` transform offset from entry → (0, 0). Eliminates direction errors when cursor lands on button / modal centers / icon flies to dock. Companion to cursor-click-ripple. Tags: cursor, landing, positioning, transform, no-reflow, anchor</anchor-at-target>
